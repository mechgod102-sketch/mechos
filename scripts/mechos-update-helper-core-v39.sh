...
# MECHOS_UPDATE_HELPER_CORE_V38_AB_ENGINE
# MECHOS_UPDATE_HELPER_CORE_V39_PACKAGE_REFRESH_STATUS
...
PACMAN_DB="${MECHOS_PACMAN_DB_DIR:-/var/lib/pacman}"
...
repair_pacman_sync_permissions(){
  local sync="$PACMAN_DB/sync"
  mkdir -p "$PACMAN_DB" "$sync"
  find "$sync" -mindepth 1 -maxdepth 1 -type d -name 'download-*' -exec rm -rf -- {} + 2>/dev/null || true
  if [ -z "${MECHOS_PACKAGE_REFRESH_TEST_MODE:-}" ]; then
    chown root:root "$PACMAN_DB" "$sync"
  fi
  chmod 0755 "$PACMAN_DB" "$sync"
}

refresh_arch_packages(){
  local log rc repaired=0 result=not-available
  if ! command -v pacman >/dev/null 2>&1; then
    printf 'PACMAN_UPDATE_ATTEMPTED=0\nPACMAN_UPDATE_RESULT=not-available\nPACMAN_UPDATE_REPAIRED=0\n'
    return 0
  fi

  log="$(mktemp /tmp/mechos-pacman-refresh.XXXXXX)"
  echo '=== Arch package refresh ==='
  if pacman -Syu --noconfirm >"$log" 2>&1; then
    ...
    rc=$?
    cat "$log"
    if grep -Fq 'Permission denied' "$log" && grep -Fq "$PACMAN_DB/sync/download-" "$log"; then
      echo 'MechOS detected a pacman sync temporary-directory permission failure.'
      echo 'Repairing only the pacman database/sync directory permissions and stale download-* directories, then retrying once.'
      repair_pacman_sync_permissions
      repaired=1
      if pacman -Syu --noconfirm >"$log" 2>&1; then
        cat "$log"
  ...
  printf 'PACMAN_UPDATE_ATTEMPTED=1\n'
  printf 'PACMAN_UPDATE_RESULT=%s\n' "$result"
  printf 'PACMAN_UPDATE_REPAIRED=%s\n' "$repaired"
  [ "$rc" -eq 0 ]
}

refresh_flatpak_packages(){
  local log rc result=not-available
  if ! command -v flatpak >/dev/null 2>&1; then
    printf 'FLATPAK_UPDATE_ATTEMPTED=0\nFLATPAK_UPDATE_RESULT=not-available\n'
    return 0
  fi

  log="$(mktemp /tmp/mechos-flatpak-refresh.XXXXXX)"
  echo '=== Flatpak package refresh ==='
  if flatpak update --system -y >"$log" 2>&1; then
    cat "$log"
    result=success
    rc=0
  else
  ...
  printf 'FLATPAK_UPDATE_ATTEMPTED=1\n'
  printf 'FLATPAK_UPDATE_RESULT=%s\n' "$result"
  [ "$rc" -eq 0 ]
}

run_post_update_package_refresh(){
  local warnings=0
  refresh_arch_packages || warnings=1
  refresh_flatpak_packages || warnings=1
  printf 'POST_UPDATE_WARNINGS=%s\n' "$warnings"
  return "$warnings"
}
...
choose_transaction(){
  local stage="$1" c
  for c in \
    "$stage/usr/local/share/mechos/update-engine/pending/mechos-update-transaction" \
    "$stage/usr/local/libexec/mechos-update-transaction-v15" \
    "$stage/usr/local/libexec/mechos-update-transaction-v14" \
    /usr/local/share/mechos/update-engine/current/mechos-update-transaction \
    /usr/local/libexec/mechos-update-transaction-v15 \
    /usr/local/libexec/mechos-update-transaction-v14 \
    /usr/local/libexec/mechos-update-transaction-v13; do
    [ -x "$c" ] || continue
    grep -Eq 'MECHOS_UPDATE_TRANSACTION_V1(4|5)' "$c" 2>/dev/null || continue
    printf '%s\n' "$c"; return 0
  done
  return 1
}
...
apply_signed(){
  ...
  echo '=== MechOS core update ==='
  printf 'MECHOS_CORE_UPDATE_STAGED=1\nSIGNATURE_VALID=1\n'
  echo "MechOS core update $latest installed successfully."

  local refresh_rc=0
  run_post_update_package_refresh || refresh_rc=$?
  if [ "$refresh_rc" -eq 0 ]; then
    echo 'Post-update package refresh completed successfully.'
  else
    echo 'MechOS core update succeeded, but one or more package refresh operations need attention.'
  fi
}
