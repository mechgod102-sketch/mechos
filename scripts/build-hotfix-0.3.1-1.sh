...
grep -Fq 'MECHOS_LEGACY_GPU_INTEGRATION_V0311' "$STAGE/usr/local/bin/mechos-gpu-setup"
grep -Fq 'PACKAGES+=(nvidia-open nvidia-utils lib32-nvidia-utils nvidia-prime)' "$STAGE/usr/local/bin/mechos-gpu-setup"
grep -Fq 'PACKAGES+=(mesa lib32-mesa vulkan-radeon lib32-vulkan-radeon libva-mesa-driver)' "$STAGE/usr/local/bin/mechos-gpu-setup"
grep -Fq 'MECHOS_LEGACY_GPU_SESSION_V0311' "$STAGE/usr/local/bin/mechscope-session"
grep -Fq 'Kernel driver in use: nvidia' "$STAGE/usr/local/bin/mechscope-session"
grep -Fq 'Kernel driver in use: nouveau' "$STAGE/usr/local/bin/mechscope-session"
grep -Fq 'Vulkan preflight failed' "$STAGE/usr/local/bin/mechscope-session"
grep -Fq 'MECHOS_LEGACY_GPU_CONTROL_V0311' "$STAGE/usr/local/libexec/mechos-031-control-suite"
