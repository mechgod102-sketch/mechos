...
is_live && { echo 'Run this rescue from the installed MechOS system, not the Live ISO.' >&2; exit 78; }
...
ROOT_UID="$(stat -c '%u' /)"
ROOT_GID="$(stat -c '%g' /)"
restore_root(){ chown "$ROOT_UID:$ROOT_GID" / 2>/dev/null || true; chmod "$ROOT_MODE" / 2>/dev/null || true; }
cleanup(){ restore_root; rm -rf "$work"; }
trap cleanup EXIT
...
latest="${meta[0]}"; url="${meta[1]}"; sha="${meta[2]}"
log "stable version: $latest"

case "$latest" in
  0.3.0-hotfix.22.*|0.3.0-hotfix.2[3-9]*|0.3.[1-9]*|0.[4-9]*|[1-9]*) ;;
  *) log "ERROR: stable release $latest does not contain the Hotfix 22 recovery baseline"; exit 20 ;;
esac
...
if [ ! -x "$CORE" ]; then
  ...
  if [ -x "$candidate" ] && grep -Fq 'MECHOS_UPDATE_TRANSACTION_V14' "$candidate" 2>/dev/null && ! grep -Fq 'MECHOS_UPDATE_TRANSACTION_V20' "$candidate" 2>/dev/null; then
    ...
    log 'ERROR: verified rsync-free staged transaction core is missing'; exit 21
  fi
fi
...
chmod "$ROOT_MODE" "$stage"; chown "$ROOT_UID:$ROOT_GID" "$stage"
...
restore_root
if [ "$rc" -ne 0 ]; then
  log "ERROR: bootstrap transaction failed with code $rc"
  tail -n 80 /var/log/mechos-update-transaction.log 2>/dev/null || true
  exit "$rc"
fi

systemctl daemon-reload >/dev/null 2>&1 || true
APPLY=/usr/local/libexec/mechos-hotfix-0.3.0-22-apply
SERVICE=/usr/lib/systemd/system/mechos-hotfix-0.3.0-22.service
[ -x "$APPLY" ] || { log 'ERROR: cumulative Hotfix 22 apply helper was not installed'; exit 22; }
[ -f "$SERVICE" ] || { log 'ERROR: cumulative Hotfix 22 boot service was not installed'; exit 23; }
...
installed="unknown"
[ -s "$RELEASE" ] && installed="$(tr -d '\r\n' <"$RELEASE")"
if [ "$installed" != "$latest" ]; then
  log "ERROR: cumulative apply returned success but release is $installed instead of $latest"
  touch "$STATE/reboot-required"
  exit 24
fi

case "$latest" in
  0.3.0-hotfix.22.3|0.3.0-hotfix.22.[4-9]*|0.3.0-hotfix.2[3-9]*)
    [ -x /usr/local/libexec/mechos-mechscope-runtime-v23 ] || { log 'ERROR: MechScope v23 runtime missing after apply'; exit 25; }
    [ -f /usr/local/libexec/mechscope-owner-v23.py ] || { log 'ERROR: preserved MechScope owner missing after apply'; exit 26; }
    ;;
esac

rm -f "$STATE/reboot-required"
log "$latest activated successfully in this rescue run"
printf '\nMechOS %s is active. Restart once to begin a clean session with the repaired runtime.\n' "$latest"
