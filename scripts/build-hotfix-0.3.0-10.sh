...
install -m 0755 "$ROOT/scripts/mechos-hotfix-0.3.0-10-apply.sh" "$STAGE/usr/local/libexec/mechos-hotfix-0.3.0-10-apply"
...
cat > "$STAGE/usr/lib/systemd/system/mechos-hotfix-0.3.0-10.service" <<'EOF'
[Unit]
Description=Apply MechOS v0.3.0 Hotfix 10 Creator visuals and MechScope launch repair
After=local-fs.target
Before=sddm.service display-manager.service
ConditionPathExists=/var/lib/mechos/installed
ConditionPathExists=!/var/lib/mechos/hotfix-0.3.0-10-applied

[Service]
Type=oneshot
ExecStart=/usr/local/libexec/mechos-hotfix-0.3.0-10-apply

[Install]
WantedBy=multi-user.target
EOF
ln -s /usr/lib/systemd/system/mechos-hotfix-0.3.0-10.service "$STAGE/etc/systemd/system/multi-user.target.wants/mechos-hotfix-0.3.0-10.service"
...
for f in \
  ...
  "$STAGE/usr/local/libexec/mechos-hotfix-0.3.0-10-apply"; do
  bash -n "$f"
done
...
data={
  'schema':1,
  'channel':'stable',
  'version':'0.3.0-hotfix.10',
  'release_name':'MechOS v0.3.0 Hotfix 10',
  'published_at':datetime.datetime.now(datetime.timezone.utc).date().isoformat(),
  'notes':'Creator graphics and MechScope launch reliability hotfix. Adds an always-available vector graphical layer to Creator Mode with app/tool/project badges and a creator hero visual while preserving live data and real backends. Repairs physical/non-virtual MechScope launches by preserving the accelerated controller path, verifying that MechScope actually appears, and falling back to the real MechScope target with hardware acceleration intact when the controller fails or produces no process. Adds detailed launcher and app startup diagnostics.',
  'bundle_url':'https://raw.githubusercontent.com/mechgod102-sketch/mechos/main/updates/bundles/MechOS-0.3.0-hotfix.10-update.tar.zst',
  'bundle_sha256':sha,
