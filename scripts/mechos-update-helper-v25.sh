...
# MECHOS_UPDATE_HELPER_V25_RELEASE_COMMIT_V1
...
commit_release_version(){
  ...
  if [[ ! "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-hotfix\.[0-9]+(\.[0-9]+)*)?$ ]]; then
    echo "Refusing invalid MechOS release version: $version" >&2
    return 18
  fi
  mkdir -p /etc/mechos
  tmp="$(mktemp /etc/mechos/.release-helper-v25.XXXXXX)"
  printf '%s\n' "$version" >"$tmp"
  chmod 0644 "$tmp"
  mv -f "$tmp" "$RELEASE"
  actual="$(tr -d '\r\n' <"$RELEASE")"
  [ "$actual" = "$version" ] || {
    echo "MechOS release commit verification failed: expected=$version actual=$actual" >&2
    return 18
  }
}
...
apply_mechos_v25(){
  ...
  work="$(mktemp -d /tmp/mechos-update-v25.XXXXXX)"
  ...
  # Hotfix 25 transactions commit this themselves. Keep this verified helper
  # commit as defense-in-depth for future compatible transaction engines.
  commit_release_version "$latest" || return $?

  mkdir -p "$STATE"
  [ "$reboot" = 1 ] && touch "$STATE/reboot-required"
  printf 'MECHOS_CORE_UPDATE_STAGED=1\n'
  printf 'MECHOS_RELEASE_COMMITTED=%s\n' "$latest"
  return 0
}
...
case "$cmd" in
  ...
  apply)
    ...
    if [ "$pkg_rc" -ne 0 ]; then
      ...
      echo 'MechOS update committed successfully. Arch/Flatpak updates can be retried after restart.' >&2
    fi
    "$CORE" status || true
    exit 0
    ;;
